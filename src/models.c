...
impl_static_array(csvtmt_column_info_array, CsvTomatoColumnInfoArray, CsvTomatoColumnInfo)
impl_static_array(csvtmt_row_array, CsvTomatoRowArray, CsvTomatoRow)
...
csvtmt_model_final(CsvTomatoModel *self) {
	...
	if (self->rows) {
		csvtmt_clear_rows(self->rows);
		csvtmt_rows_del(self->rows);
	}
}
...
type_parse_column(
	...
	return;
overflow:
	csvtmt_error_push(error, CSVTMT_ERR_BUF_OVERFLOW, "buffer overflow in parse type column");
}
...
header_parse_types(
	...
	for (size_t i = 0; i < row->len; i++) {
		if (self->types_len >= csvtmt_numof(self->types)) {
			csvtmt_error_push(error, CSVTMT_ERR_BUF_OVERFLOW, "header types overflow");
			return;
		}
		CsvTomatoColumnType *type = &self->types[self->types_len++];
		type_parse_column(type, row->columns[i], i, error);
		if (error->error) {
			csvtmt_error_push(error, CSVTMT_ERR_PARSE, "failed to parse header column");
			return;
		}
	}
}
...
csvtmt_header_read_from_string(CsvTomatoHeader *self, const char *p, CsvTomatoError *error) {
	CsvTomatoRow row = {0};
	p = csvtmt_row_parse_string(&row, p, error);
	if (error->error) {
		return NULL;
	}

	header_parse_types(self, &row, error);
	if (error->error) {
		goto failed_to_parse_types;
	}

	csvtmt_row_final(&row);
	return p;

failed_to_parse_types:
	csvtmt_error_push(error, CSVTMT_ERR_BUF_OVERFLOW, "failed to header types");
	return NULL;
}
...
csvtmt_header_read_from_stream(CsvTomatoHeader *self, FILE *fp, CsvTomatoError *error) {
	...
	csvtmt_row_parse_stream(&row, fp, error);
	if (error->error) {
		fclose(fp);
		return;
	}
...
csvtmt_header_read_from_table(CsvTomatoHeader *self, const char *table_path, CsvTomatoError *error) {
	...
	if (!fp) {
		csvtmt_error_push(error, CSVTMT_ERR_FILE_IO, "failed to open table: %s: %s", table_path, strerror(errno));
		return;
	}
...
failed_to_open_file:
	csvtmt_error_push(error, CSVTMT_ERR_FILE_IO, "failed to open id file: %s", strerror(errno));
	return 0;
}
...
csvtmt_parse_row_from_mmap(CsvTomatoModel *model, CsvTomatoError *error) {
	...
	model->mmap.cur = (char *) csvtmt_row_parse_string(&model->row, model->mmap.cur, error);
	if (error->error) {
		return;
	}
}
...
type_gen_column_default_value(
	...
	return;
invalid_type_def:
	csvtmt_error_push(error, CSVTMT_ERR_EXEC, "failed to generate column default value");
	return;
fail_gen_id:
	return;
}
...
csvtmt_update_all(CsvTomatoModel *model, CsvTomatoError *error) {
	...
	csvtmt_store_column_infos(model, &infos, model->update_set_key_values, model->update_set_key_values_len, error);
	if (error->error) {
		return CSVTMT_ERROR;
	}
	...
	for (;; nline++) {
		csvtmt_row_final(&row);
		int ret = csvtmt_row_parse_stream(&row, fin, error);
		if (error->error) {
		...
		if (nline != 0) {
			for (size_t ii = 0; ii < infos.len; ii++) {
				CsvTomatoColumnInfo *info = &infos.array[ii];
				...
				for (size_t ci = 0; ci < row.len; ci++) {
					if (info->index == ci) {
						...
						csvtmt_row_set_clone(&row, info->index, s, error);
						free(s);
						if (error->error) {
		...
		csvtmt_row_append_to_stream(&row, fout, true, error);
		if (error->error) {
...
failed_to_value_to_string:
	csvtmt_error_push(error, CSVTMT_ERR_FILE_IO, "failed to value to string");
	return CSVTMT_ERROR;
failed_to_rename_csv_file:
	csvtmt_error_push(error, CSVTMT_ERR_FILE_IO, "failed to rename csv file");
	return CSVTMT_ERROR;
...
failed_to_open_tmp_file:
	fclose(fin);
	csvtmt_error_push(error, CSVTMT_ERR_FILE_IO, "failed to open tmp file: %s: %s", tmp_path, strerror(errno));
	return CSVTMT_ERROR;
failed_to_parse_stream:
	fclose(fin);
	fclose(fout);
	csvtmt_error_push(error, CSVTMT_ERR_EXEC, "failed to parse stream");
	return CSVTMT_ERROR;
failed_to_append_to_stream:
	fclose(fin);
	fclose(fout);
	csvtmt_error_push(error, CSVTMT_ERR_EXEC, "failed to append to stream");
	return CSVTMT_ERROR;
failed_to_replace:
	fclose(fin);
	fclose(fout);
	csvtmt_error_push(error, CSVTMT_ERR_EXEC, "failed to replace column");
	return CSVTMT_ERROR;
}
	...
	for (size_t ti = 0; ti < types_len; ti++) {
		...
		for (size_t kvi = 0; kvi < kvs_len; kvi++) {
			...
			if (!strcmp(type->type_name, kv->key)) {
				CsvTomatoColumnInfo info = {
					.key = type->type_name,
					.index = ti,
					.value = kv->value,
				};
				csvtmt_column_info_array_push(infos, info, error);
				if (error->error) {
					goto array_overflow;
				}
				break;
...
csvtmt_store_selected_columns(CsvTomatoModel *model, CsvTomatoRow *row, CsvTomatoError *error) {
	...
	if (tlen != row->len) {
		csvtmt_error_push(error, CSVTMT_ERR_EXEC, "invalid row length");
		return;
	}
			...
			for (size_t ci = 0; ci < clen; ci++) {
				...
				if (!strcmp(type->type_name, col)) {
					if (model->selected_columns_len >= csvtmt_numof(model->selected_columns)) {
						csvtmt_error_push(error, CSVTMT_ERR_BUF_OVERFLOW, "selected columns overflow");
						return;
					}
...
csvtmt_replace_row(
	...
	for (size_t i = 0; i < infos->len; i++) {
		CsvTomatoColumnInfo *info = &infos->array[i];
...
/*
static CsvTomatoColumnInfo *
where_match(CsvTomatoColumnInfoArray *where_infos, CsvTomatoRow *row) {
	for (size_t ii = 0; ii < where_infos->len; ii++) {
		CsvTomatoColumnInfo *winfo = &where_infos->array[ii];
...
csvtmt_open_mmap(
	...
	errno = 0;
	model->mmap.fd = open(table_path, fd_mode);
	if (model->mmap.fd == -1) {
		goto failed_to_open_table;
	}
	...
	model->mmap.ptr = mmap(NULL, model->mmap.size, mmap_mode, MAP_SHARED, model->mmap.fd, 0);
	...
	return;
failed_to_open_table:
	csvtmt_error_push(error, CSVTMT_ERR_FILE_IO, "failed to open table");
	return;
failed_to_stat:
	csvtmt_error_push(error, CSVTMT_ERR_FILE_IO, "failed to stat");
	return;
failed_to_mmap:
	csvtmt_error_push(error, CSVTMT_ERR_MEM, "faled to mmap");
	return;
}
...
csvtmt_open_mmap_for_read(
	...
	csvtmt_open_mmap(model, table_path, O_RDONLY, PROT_READ, error);
}
...
csvtmt_open_mmap_for_read_write(
	...
	csvtmt_open_mmap(model, table_path, O_RDWR, PROT_READ | PROT_WRITE, error);
}
...
csvtmt_delete(CsvTomatoModel *model, CsvTomatoError *error) {
	...
	if (has_where) {
		...
		CsvTomatoColumnInfoArray where_infos = {0};

		csvtmt_store_column_infos(model, &where_infos, model->where_key_values, model->where_key_values_len, error);
		if (error->error) {
			goto array_overflow;
		}

		CsvTomatoRow row = {0};
		char *p = model->mmap.cur;

		for (; *p; ) {
			char *head = p;
			p = (char *) csvtmt_row_parse_string(&row, p, error);
			if (error->error) {
			...
			CsvTomatoColumnInfo *winfo = where_match(&where_infos, &row);
			if (winfo) {
				// match
				if (*head == '"') {
					head++;
	...
	return CSVTMT_DONE;
failed_to_parse_csv:
	csvtmt_error_push(error, CSVTMT_ERR_EXEC, "failed to parse csv line");
	return CSVTMT_ERROR;
array_overflow:
	csvtmt_error_push(error, CSVTMT_ERR_EXEC, "array overflow");
	return CSVTMT_ERROR;
	*/
	return 0;
}
...
csvtmt_update(CsvTomatoModel *model, CsvTomatoError *error) {
	...
	if (has_where) {
				...
				for (size_t ci = 0; ci < row.len; ci++) {
					...
					if (ci == winfo->index) {
						...
						if (value_eq(&winfo->value, &val)) {
							...
							// replace
							csvtmt_replace_row(model, &row, &set_infos, error);
							row_array_push(&lines, row, error);
							if (error->error) {
								goto array_overflow;
							}
		...
		for (size_t i = 0; i < lines.len; i++) {
			csvtmt_row_final(&lines.array[i]);
		}
...
failed_to_replace_update:
	csvtmt_error_push(error, CSVTMT_ERR_EXEC, "failed to replace update");
	return CSVTMT_ERROR;
invalid_key_values_type:
	csvtmt_error_push(error, CSVTMT_ERR_EXEC, "invalid update key. \"%s\" is not in header types", not_found);
	return CSVTMT_ERROR;
failed_to_stat:
	csvtmt_error_push(error, CSVTMT_ERR_EXEC, "failed to get table size: %s", strerror(errno));
	return CSVTMT_ERROR;
failed_to_mmap:
	csvtmt_error_push(error, CSVTMT_ERR_EXEC, "failed to mmap. %s", strerror(errno));
	return CSVTMT_ERROR;
failed_to_open_table:
	csvtmt_error_push(error, CSVTMT_ERR_EXEC, "failed to open table %s. %s", model->table_path, strerror(errno));
	return CSVTMT_ERROR;
failed_to_parse_csv:
	csvtmt_error_push(error, CSVTMT_ERR_EXEC, "failed to parse csv line");
	return CSVTMT_ERROR;
...
failed_to_append_lines:
	csvtmt_error_push(error, CSVTMT_ERR_EXEC, "failed to append csv lines");
	return CSVTMT_ERROR;
	*/
	return 0;
}
...
csvtmt_select(CsvTomatoModel *model, CsvTomatoError *error) {
	...
	if (model->mmap.fd == 0) {
		csvtmt_header_read_from_table(&model->header, model->table_path, error);
		if (error->error) {
		...
		model->mmap.cur = (char *) csvtmt_row_parse_string(&row, model->mmap.cur, error);
		if (error->error) {
			goto failed_to_parse_csv;
		}
		csvtmt_row_final(&row);
...
failed_to_store_colinfo:
	csvtmt_error_push(error, CSVTMT_ERR_EXEC, "failed to store column info");
	return CSVTMT_ERROR;
failed_to_header_read:
...
failed_to_stat:
	...
	return CSVTMT_ERROR;
invalid_column:
	csvtmt_error_push(error, CSVTMT_ERR_EXEC, "invalid column name. \"%s\" is not in header types", not_found);
	return CSVTMT_ERROR;
failed_to_open_table:
	csvtmt_error_push(error, CSVTMT_ERR_EXEC, "failed to open table %s. %s", model->table_path, strerror(errno));
	return CSVTMT_ERROR;
...
failed_to_wrap_column:
	csvtmt_error_push(error, CSVTMT_ERR_EXEC, "failed to wrap column");
	cleanup();
	return CSVTMT_ERROR;
failed_to_allocate_buffer:
	csvtmt_error_push(error, CSVTMT_ERR_EXEC, "failed to allocate buffer");
	cleanup();
	return CSVTMT_ERROR;
failed_to_gen_type_string:
	csvtmt_error_push(error, CSVTMT_ERR_EXEC, "failed to generate type column value");
	cleanup();
	return CSVTMT_ERROR;
failed_to_header_read:
	cleanup();
...
invalid_column:
	...
	return CSVTMT_ERROR;
invalid_values_len:
	csvtmt_error_push(error, CSVTMT_ERR_EXEC, "invalid values length. column names len is \"%ld\" but values length is \"%ld\"", model->column_names_len, model->values_len);
	cleanup();
	return CSVTMT_ERROR;
invalid_value_kind:
	csvtmt_error_push(error, CSVTMT_ERR_EXEC, "invalid value kind");
	cleanup();
	return CSVTMT_ERROR;
failed_to_open_table:
	csvtmt_error_push(error, CSVTMT_ERR_EXEC, "failed to open table %s. %s", model->table_path, strerror(errno));
...
values_index_out_of_ragen:
	csvtmt_error_push(error, CSVTMT_ERR_EXEC, "values index out of range");
	cleanup();
	return CSVTMT_ERROR;
}
...
csvtmt_find_type_index(CsvTomatoModel *model, const char *type_name) {
	for (int i = 0; i < model->header.types_len; i++) {
...
csvtmt_append_rows_to_table(
	...
	for (size_t i = 0; i < rows->len; i++) {
		CsvTomatoRow *row = &rows->array[i];
		csvtmt_row_append_to_stream(row, fp, wrap, error);
		if (error->error) {
